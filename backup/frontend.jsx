...
const MY_USER_ID = 'student_001';

const initialScripts = [
    { id: 1, title: 'Uvod u Mikroekonomiju', course: 'Mikroekonomija', authorId: 'creator_a', price: 15, rating: 4.8, numRatings: 22, description: 'Sažeti pregled temeljnih principa mikroekonomije za prvu godinu.', purchasedBy: [MY_USER_ID], content: 'Ovo je dešifrirana skripta za Mikroekonomiju...' },
    { id: 2, title: 'Osnove Računovodstva - Bilanca', course: 'Računovodstvo', authorId: 'creator_b', price: 20, rating: 4.2, numRatings: 15, description: 'Detaljna analiza bilance i osnovnih knjigovodstvenih operacija.', purchasedBy: [], content: 'Ovo je dešifrirana skripta za Računovodstvo...' },
    { id: 3, title: 'Kvantitativne Metode 1', course: 'Kvantitativne Metode', authorId: 'creator_c', price: 10, rating: 5.0, numRatings: 5, description: 'Formule i primjeri za sve ispite iz Kvantitativnih Metoda (1. dio).', purchasedBy: [], content: 'Ovo je dešifrirana skripta za Kvantitativne Metode...' },
];
...
const App = () => {
    ...
    const [currentView, setCurrentView] = useState('dashboard'); // 'dashboard', 'details', 'upload', 'wallet'
    const [selectedScript, setSelectedScript] = useState(null);
    const [searchQuery, setSearchQuery] = useState('');
    const [showModal, setShowModal] = useState(null); // 'purchase' | 'rating'
    ...
    const handleWalletConnect = () => {
        ...
        // Simulacija Airdrop-a
        setSvlBalance(INITIAL_SVL_AIRDROP);
        setCurrentView('dashboard');
    };

    const handleAirdrop = () => {
        setSvlBalance(svlBalance + INITIAL_SVL_AIRDROP);
    };

    // --- LOGIKA POTROSACA (KUPOVINA) ---

    const handlePurchase = (script) => {
        if (svlBalance < script.price) {
            console.error('Nedovoljan SVL saldo za kupnju!');
            return;
        }

        // 1. Token Transakcija (simulacija)
        const price = script.price;
        const authorId = script.authorId;
        const buyerId = MY_USER_ID;

        // Dijeljenje tokena (100% autoru u ovoj simulaciji)
        setSvlBalance(prev => prev - price);

        // Ažuriranje skripte: Dodavanje kupca i označavanje kao kupljene
        setScripts(prevScripts =>
            prevScripts.map(s =>
                s.id === script.id
                    ? { ...s, purchasedBy: [...s.purchasedBy, buyerId] }
                    : s
            )
        );

        // Simulacija primanja tokena od strane autora (za potrebe prikaza u ne-prijavljenom stanju, ovo se ne radi u stvarnoj aplikaciji ovako)
        console.log(`Simulacija: ${price} $SVL$ prebačeno autoru ${authorId}.`);

        setSelectedScript({ ...script, purchasedBy: [...script.purchasedBy, buyerId] });
        setShowModal(null);
    };

    const handleRating = (scriptId, ratingValue, reviewText) => {
        // 1. Ažuriranje prosječne ocjene
        setScripts(prevScripts => prevScripts.map(s => {
            if (s.id === scriptId) {
                // Nova prosječna ocjena (jednostavna formula za simulaciju)
                const newTotalRating = (s.rating * s.numRatings) + ratingValue;
                const newNumRatings = s.numRatings + 1;
                const newRating = newTotalRating / newNumRatings;

                // 2. Akumulacija Reputacijskog Kredita (RC)
                setReputationCredit(prev => prev + 5); // Dajemo +5 RC za recenziju

                return { ...s, rating: newRating.toFixed(1), numRatings: newNumRatings, hasRated: true };
            }
            return s;
        }));

        setSelectedScript(prev => ({ ...prev, hasRated: true, rating: ratingValue }));
        setShowModal(null);
    };
    ...
    const handleUpload = (newScript) => {
        // 1. Mintanje NFT-a (simulacija) i pohrana na IPFS
        const id = scripts.length + 1;
        const newScriptWithMeta = {
            id,
            ...newScript,
            authorId: MY_USER_ID,
            rating: 0,
        ...
        setScripts(prev => [...prev, newScriptWithMeta]);
        setCurrentView('dashboard');
    };
    ...
    const isScriptPurchased = (script) => script?.purchasedBy?.includes(MY_USER_ID);
    ...
    const RatingModal = ({ script, onClose }) => {
        ...
        const submitReview = () => {
            ...
            handleRating(script.id, rating, review);
        };
...
const Dashboard = ({ svlBalance, reputationCredit, onUploadClick, onAirdrop, scripts, onScriptSelect, searchQuery, setSearchQuery }) => {
    return (
        <div>
            ...
            <ScriptList scripts={scripts} onScriptSelect={onScriptSelect} myUserId={MY_USER_ID} />
            ...
            {scripts.map(script => (
                ...
                >
                    ...
                    <div className="flex justify-between items-center pt-3 border-t border-gray-100 dark:border-gray-700">
                        {script.purchasedBy.includes(myUserId) ? (
                        ...
                        <span className="text-xs text-gray-500 dark:text-gray-400">Autor: {script.authorId}</span>
                    </div>
                </div>
            ))}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 sm:p-10 rounded-xl shadow-2xl max-w-4xl mx-auto">
            ...
            <header className="border-b pb-4 mb-6 border-gray-200 dark:border-gray-700">
                ...
                <div className="flex items-center mt-2 space-x-4">
                    ...
                    <span className="text-sm text-gray-500 dark:text-gray-400">Autor: {script.authorId}</span>
                </div>
            </header>
